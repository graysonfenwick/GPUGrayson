...
static __device__ void koggeStoneScan(int *, int *);
...
__global__ void d_classifyKernel(float * comparisons, float * normHistograms, int * histograms) 
{
    ...
    while (bins > 0)
    {  
       koggeStoneScan(&histograms[blockIdx.x * TOTALBINS + i * blockDim.x], &prevSz);
       __syncthreads();
       if (threadIdx.x == 0) 
       {
...
__device__ void koggeStoneScan(int * histogram, int * histSz)
{
   int tmp;
   __shared__ int shHistogram[CLASSBLOCKDIM];
   int tid = threadIdx.x;

   shHistogram[tid] = histogram[tid];
   __syncthreads();
   for (unsigned int stride = 1; stride < CLASSBLOCKDIM; stride *= 2)
   {
      __syncthreads();
       if (tid >= stride)
       {
           tmp = shHistogram[tid - stride];
       }
       __syncthreads();
       if (tid >= stride)
       {
    ...
    __syncthreads();
    if (tid == 0) (*histSz) = shHistogram[CLASSBLOCKDIM - 1]; 
}
