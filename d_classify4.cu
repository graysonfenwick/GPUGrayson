...
static __device__ void bankBelloch(int *histogram, int *histSz);
...
__global__ void d_classifyKernel(float * comparisons, float * normHistograms, int * histograms) 
{
    ...
    while (bins > 0)
    {  
       bankBelloch(&histograms[blockIdx.x * TOTALBINS + i * blockDim.x * factor], 
                   &prevSz);
       __syncthreads();
       if (threadIdx.x == 0) 
       {
...
__device__ void bankBelloch(int *histogram, int *histSz)
{
